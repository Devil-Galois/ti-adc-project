...
module tb_calib1();
    ...
    initial begin
        if ($test$plusargs("dump")) begin
            $dumpfile("build/tb_calib1.vcd");   // save the waveform to be checked
    ...
    initial begin
        ...
        $display("Loading Calibration Data: tran_data_calib1.txt");
        $readmemb("bin/tran_data_calib1.txt", bit_pool);
        $display("Loading Test Data: test_data.txt");
        $readmemb("bin/test_data.txt", tst_pool);  // original test data input 8bits
        ...
        f_reg = $fopen("bin/reg_cal1.txt", "w");
        ...
        $fclose(f_reg);
        $display("Registers saved to bin/reg_cal1.txt");

        // test data input and calibration
        f_test_out = $fopen("bin/test_calib1_dec.txt", "w");
        if (f_test_out == 0) begin $display("Error: Cannot open test_calib1_dec.txt"); $finish; end
        ...
        $fclose(f_test_out);
        $display("Calibration and Test complete. Calibrated test data saved to bin/test_calib1_dec.txt");
